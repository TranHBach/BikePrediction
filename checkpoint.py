...
def get_all_components(product_id, bom_data, multiplier=1, memo=None):
    """
    Recursively get all components needed for a product
    """
    if memo is None:
        memo = {}
    
    # Filter BOM data for current product
    # First try with EndDate filter
    components = bom_data[
        (bom_data['ProductAssemblyID'] == product_id) & 
        (pd.isna(bom_data['EndDate']))
    ]
    
    # If no active components found, try without EndDate filter
    if len(components) == 0:
        components = bom_data[bom_data['ProductAssemblyID'] == product_id]
        
        # If still no components, return memo
        if len(components) == 0:
            return memo
...
print(f"\nTotal bikes predicted for next 2 weeks: {total_bikes}")

# Calculate components needed based on predictions
total_components_needed = {}

# For each bike with predicted sales
for bike_id, predicted_qty in all_predictions.items():
    # Get all components needed for this bike
    components = get_all_components(bike_id, bill_of_material)
    
    # Multiply component quantities by predicted bike quantity
    for component_id, qty in components.items():
        total_qty = qty * predicted_qty
        total_components_needed[component_id] = total_components_needed.get(component_id, 0) + total_qty

# After calculating total_components_needed, add this filter:
# Get all components that have ever been purchased
purchased_components = purchase_order_detail['ProductID'].unique()

# Filter total_components_needed to only include components with purchase history
# total_components_needed = {
#     component_id: qty 
#     for component_id, qty in total_components_needed.items()
#     if component_id in purchased_components
# }

print("\nAdjusted Component Requirements After Considering Inventory:")
print("==============================================================")
final_results = []
...
for component_id, qty_needed in total_components_needed.items():
    # Get current inventory across all locations
    current_inventory = product_inventory[
        product_inventory['ProductID'] == component_id
    ]['Quantity'].sum()
    
    # Calculate the additional quantity needed
    reorder_qty = max(0, qty_needed - current_inventory)
    
    # Only process if we need to reorder
    if reorder_qty > 0:
        # Calculate reasonable purchase order quantity
        purchase_qty = calculate_purchase_order(component_id, reorder_qty, purchase_order_detail)
        unit_cost = calculate_component_cost(component_id, purchase_order_detail)
        total_cost = unit_cost * purchase_qty
        
        final_results.append({
            'Product': component_id,
            'Items_needed': round(purchase_qty),
            'Unit_Cost': round(unit_cost, 2),
            'Total_Cost': round(total_cost, 2)
        })
        print(f"Product {component_id}: needs {round(purchase_qty)} additional units, Cost: ${round(total_cost, 2)}")
    else:
        print(f"Product {component_id}: No additional units needed (Current Inventory: {current_inventory})")

# Optionally, sort the results by the number of items needed in descending order
final_results = sorted(final_results, key=lambda x: x['Items_needed'], reverse=True)

# Optional: Save the results to a CSV file
pd.DataFrame(final_results).to_csv('component_requirements.csv', index=False)

# Add at the end of your script, after all calculations
save_cache_to_file()

# Update the CSV output to include shipping recommendation
pd.DataFrame(final_results).to_csv('component_requirements.csv', index=False)